...
- **The published data itself is world-readable.** The embedded JSON carries the full raw Zoho records — `notes` free text, entry numbers, branch/location IDs, every amount — and a Vercel production URL is public by default. If that is not acceptable, turn on Vercel **Deployment Protection** (password or SSO) for the project; nothing in this repo restricts who can read a deployed dashboard.
